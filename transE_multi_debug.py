...
class TransE:
    ...
    def __init__(self, data_dir, embed_dim=100, fanout_thresh=2):
    ...
    def train(self, inputs, regularizer_weight=1., scope=None):
        with tf.variable_scope(scope or type(self).__name__) as scp:
            ...
            loss = tf.reduce_mean(tf.maximum(pos_list_score - neg_list_1_score + 2, 0)) + \
                    tf.reduce_mean(tf.maximum(pos_list_score - neg_list_2_score + 2, 0)) + \
                    tf.reduce_mean(tf.maximum(pos_list_score - neg_list_3_score + 2, 0)) + \
                    tf.reduce_mean(tf.maximum(pos_list_score - neg_list_4_score + 2, 0)) + \
                    tf.reduce_mean(tf.maximum(pos_list_score - neg_list_5_score + 2, 0))
...
def data_generator_func(in_queue, out_queue, tr_h, hr_t, n_entity, neg_sample, n_rel):
    while True:
        dat = in_queue.get()
        if dat is None:
            break
        ...
        #         for pos_head in tr_h[htr[idx, 1]][htr[idx, 2]]:
        #             for neg_head in neg_h_list:
        #                 pos_neg_list.append((pos_head,htr[idx, 2],htr[idx, 1],neg_head,htr[idx, 2],htr[idx, 1]))
        #                 assert pos_neg_list[-1][1] < n_rel
        #                 assert pos_neg_list[-1][4] < n_rel

        #     else:  # h r predict t
        #         neg_t_list = random.sample([x for x in range(n_entity) if x not in hr_t[htr[idx, 0]][htr[idx, 2]]],neg_sample)

        #         for pos_tail in hr_t[htr[idx, 0]][htr[idx, 2]]:
        #             for neg_tail in neg_t_list:
        #                 pos_neg_list.append((htr[idx, 0], htr[idx, 2],pos_tail,htr[idx, 0], htr[idx, 2],neg_tail))
        #                 assert pos_neg_list[-1][1] < n_rel
        #                 assert pos_neg_list[-1][4] < n_rel
        ...
        out_queue.put((np.asarray(pos_neg_list, dtype=np.int32)))
...
def main(_):
    ...
    model = TransE(args.data_dir, embed_dim=args.dim, fanout_thresh=args.fanout_thresh)
    ...
    with tf.Session() as session:
        try:
            ...
            for var in tf.all_variables():
                print var.name, var, var.get_shape()
        ...
        for i in range(args.n_generator):
            data_generators.append(Process(target=data_generator_func, args=(
                raw_training_data_queue, training_data_queue, model.tr_h, model.hr_t, model.n_entity, args.neg_sample, model.n_relation)))
        ...
        for n_iter in range(iter_offset, args.max_iter):
            ...
            # x = model.ent_embedding
            # l2norm_inv = tf.rsqrt(tf.reduce_sum(x * x, 1, keep_dims=True))
            # f1.write l2norm_inv.eval()
            # x = x*l2norm_inv
            # session.run(model.ent_embedding.assign(x))
            ...
            for batch_id in range(nbatches_count):
                ...
                pos_neg_list = training_data_queue.get()
                # data_e = training_data_list[batch_id]
                #print data_e
                run_metadata = tf.RunMetadata()
                l, _ = session.run(
                    [train_loss, train_op], {train_pos_neg_list : pos_neg_list},options=tf.RunOptions(trace_level=tf.RunOptions.FULL_TRACE),
                                                                                run_metadata=run_metadata)
                trace = timeline.Timeline(step_stats=run_metadata.step_stats)

                trace_file = open('timeline.ctf.json', 'a')
                ...
                if batch_id % args.save_per_batch == 0 and batch_id > 0: # save after every x (=1000 default) batches
                    for data_func, test_type in zip([model.validation_data, model.testing_data], ['VALID', 'TEST']):
                        ...
                        f1.write("waiting for worker finishes their work")
                        evaluation_queue.join()
                        f1.write("all worker stopped.")
            ...
            f1 = open('%s/%s.txt' % (args.save_dir, args.outfile_prefix),'a')
            f1.write("")
            f1.write("iter %d avg loss %.5f, time %.3f" % (n_iter, accu_loss / ninst, timeit.default_timer() - start_time))
