...
class TransE:
    ...
    def __init__(self, data_dir, embed_dim=100, fanout_thresh=2):
        ...
        with tf.device('/cpu'):
            ...
            self.__train_triple = triples_arr[:int(0.99997*idx.shape[0])]
            self.__valid_triple = triples_arr[int(0.99997*idx.shape[0]):int(0.99998*idx.shape[0])]
            self.__test_triple = triples_arr[int(0.99998*idx.shape[0]):]
...
def main(_):
    ...
    # parser.add_argument('--regularizer_weight',dest='regularizer_weight',type=float,help='L2 regularizer_weight')
