...
class TransE:
    ...
    @property
    def id_entity_map(self):
        return self.__id_entity_map

    @property
    def id_rel_map(self):
        return self.__id_relation_map
    ...
    def __init__(self, data_dir, embed_dim=100, fanout_thresh=2,eval_batch=32):
        ...
        with tf.device('/cpu'):
            ...
            with open('id_entity_map.pickle','wb') as f2:
                pickle.dump(self.__id_entity_map, f2)

            with open('id_rel_map.pickle','wb') as f2:
                pickle.dump(self.__id_relation_map, f2)
...
def map_ops(model):
    return model.id_entity_map, model.id_rel_map
...
def main(_):
    ...
    with tf.Session() as session:
        ...
        with tf.device('/cpu'):
            ...
            pos_neg_list = training_data_pos_neg_list[0]
            ent_embedding, rel_embedding = session.run(get_embedding_op,{train_pos_neg_list : pos_neg_list})
            # id_entity_map, id_rel_map = session.run(get_map_op,{train_pos_neg_list : pos_neg_list})
            ...
            np.save('ent_embed.pkl',np.asarray(ent_embedding))
            np.save('rel_embed.pkl',np.asarray(rel_embedding))
