    ...
    def __init__(self, data_dir, embed_dim=100, fanout_thresh=2,eval_batch=32):
        ...
        with tf.device('/cpu'):
            ...
            def load_triple():
                ...
                for i in xrange(11796300): #11963105
            ...
            self.__train_triple = triples_arr[:int(0.01*idx.shape[0])]
...
def test_evaluation(testing_data, head_pred, tail_pred, hr_t, tr_h):
    ...
    # h_list = testing_data[:,0]
    # t_list = testing_data[:,1]
    # r_list = testing_data[:,2]
    ...
    for i in range(len(testing_data)):
        ...
        if h not in head_pred[i]:
            mr = 2 * len(head_pred[i])
        else:    
            mr = head_pred[i].index(h)
        mean_rank_h.append(mr)

        if t not in tail_pred[i]:
            mr = 2 * len(tail_pred[i])
        else:
            mr = tail_pred[i].index(t)
        mean_rank_t.append(mr)

        # head_pred_filt = head_pred[i]

        # if h not in head_pred_filt:
        #     fmr = 2 * len(head_pred_filt)
        # else:
        #     if t in tr_h and r in tr_h[t]:
        #         for x in (set(tr_h[t][r]) & set(head_pred_filt)):
        #             head_pred_filt.remove(x)

        #     if h not in head_pred_filt:
        #         fmr = 2 * len(head_pred_filt)
        #     else:
        #         fmr = head_pred_filt.index(h)
        #     filtered_mean_rank_h.append(fmr)
        # # filtered mean rank

        # tail_pred_filt = tail_pred[i]
        # if t not in tail_pred_filt:
        #     fmr = 2 * len(tail_pred_filt)
        # else:
        #     if h in hr_t and r in hr_t[h]:
        #         for x in (set(hr_t[h][r]) & set(tail_pred_filt)):
        #             tail_pred_filt.remove(x)

        #     if t not in tail_pred_filt:
        #         fmr = 2 * len(tail_pred_filt)
        #     else:
        #         fmr = tail_pred_filt.index(t)
        #     filtered_mean_rank_t.append(fmr)
...
def main(_):
    ...
    parser.add_argument('--eval_after',dest='eval_after',type=int,help='Evaluate after this many no. of epochs', default=0)
        ...
        for n_iter in range(iter_offset, args.max_iter):            
            ...
            with tf.device('/cpu'):
                if n_iter > args.eval_after and (n_iter % args.eval_per == 0 or n_iter == args.max_iter - 1):
                    ...
                    print 'Index creation completed time taken = %f' % (timeit.default_timer() - st_1)
                    ...
                    for data_func, test_type in eval_dict:
                        ...
                        for testing_data in data_func(batch_size=args.eval_batch):
                            ...
                            for i in xrange(testing_data.shape[0]):
                                ...
                                head_ids.append(t.get_nns_by_vector((ent_embedding[testing_data[i,0],:] + rel_embedding[testing_data[i,2],:]), n,search_k))
                                tail_ids.append(t.get_nns_by_vector((ent_embedding[testing_data[i,1],:] - rel_embedding[testing_data[i,2],:]), n,search_k))
                                # except:
                                #     print 'i = %d' % i
                        ...
                        print 'time taken for head id and tail id calc = %f' % (timeit.default_timer() - st)

                        while evaluation_count > 0:
                            evaluation_count -= 1
                            print 'eval test_type: %s, batch id: %d' % (test_type, evaluation_count)
                        ...
                        print 'time taken for metric computation = %f' % (timeit.default_timer() - et)
