...
class TransE:
    ...
    def corrupt_sample(self,htr):
        ...
        for idx in range(htr.shape[0]):
            ...
            if(prob < 0.5):
                neg_t = -1
                while neg_t < 0:
                    tmp = np.random.randint(0,self.__n_entity_type)
    ...
    def __init__(self, data_dir, embed_dim=100, fanout_thresh=2,eval_batch=32):
        ...
        with tf.device('/cpu'):
            ...
            with open('/dccstor/cssblr/vardaan/dialog-qa/child_par_dict_immed.json','r') as f1:
                child_par_dict_immed = json.load(f1)
            # print len(wikidata)
            ...
            #         triples_arr.append([random.randint(0,self.__n_entity_type-1), random.randint(0,self.__n_entity_type-1), random.randint(0,self.__n_relation-1)])
            ...
            for tpl in triples:
                curr_tpl = [self.__id_entity_map[tpl[0]], self.__id_entity_map[tpl[1]], self.__id_relation_map[tpl[2]]]
                nlevel = 6
                while nlevel > 0:
                    if curr_tpl[0] in child_par_dict_immed and curr_tpl[1] in child_par_dict_immed:
                        curr_tpl[0] = child_par_dict_immed[curr_tpl[0]]
                        curr_tpl[1] = child_par_dict_immed[curr_tpl[1]]
                        entity_type_set.update([curr_tpl[0], curr_tpl[1]])
                    else:
                        break
                    nlevel -= 1

            self.entity_type_id_map = {ent_type:i for i,ent_type in enumerate(list(entity_type_set))}

            self.id_entity_type_map = {i:ent_type for i,ent_type in enumerate(list(entity_type_set))}

            self.__n_entity_type = len(self.entity_type_id_map.keys())
            ...
            for tpl in triples:
                ...
                while nlevel > 0:
                    if curr_tpl[0] in child_par_dict_immed and curr_tpl[1] in child_par_dict_immed:
                        curr_tpl[0] = child_par_dict_immed[curr_tpl[0]]
                        curr_tpl[1] = child_par_dict_immed[curr_tpl[1]]
                        type_triples.add((self.entity_type_id_map[curr_tpl[0]], self.entity_type_id_map[curr_tpl[1]], self.__relation_id_map[curr_tpl[2]]))
                    else:
                        break
                    nlevel -= 1
            type_triples_list = [list(x) for x in type_triples]
            triples_arr = np.asarray(type_triples_list,dtype=np.int32)
        ...
        #     self.__n_entity_type = len(f.readlines())
        ...
        # ********************************************************************************************
            print("N_ENTITY: %d" % self.__n_entity_type)
        ...
        # with tf.device('/cpu'):
        self.ent_embeddings = tf.get_variable("ent_embedding", [self.__n_entity_type, embed_dim],
    ...
    def test(self, inputs, scope=None):
        with tf.variable_scope(scope or type(self).__name__) as scp:
            ...
            # _, tail_ids = tf.nn.top_k(trh_res, k=self.__n_entity_type)

            # hrh_res = tf.matmul(t-r, ent_mat)            
            # _, head_ids = tf.nn.top_k(hrh_res, k=self.__n_entity_type)
...
def main(_):
    ...
    parser.add_argument("--eval_per", dest='eval_per', type=int, help="Evaluate every x iteration", default=3)
        ...
        for n_iter in range(iter_offset, args.max_iter):            
            ...
            with tf.device('/cpu'):
                if n_iter > args.eval_after and (n_iter % args.eval_per == 0 or n_iter == args.max_iter - 1):
                    ...
                    if n_iter == args.max_iter - 1:
                        eval_dict = zip([model.validation_data, model.testing_data], ['VALID', 'TEST'])
                    else:
                    ...
                    for data_func, test_type in eval_dict:
                        ...
                        if test_type == 'VALID':
                            filtered_mean_rank = (np.mean(accu_filtered_mean_rank_t) + np.mean(accu_mean_rank_h))/2.0
