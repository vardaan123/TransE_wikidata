...
def data_generator_func(dat, tr_h, hr_t, n_entity, neg_weight):
        # [head(tail), relation, #of_total_positive_candidates, positive_instances..., negative_instances...]
    hr_tlist = list()
    hr_tweight = list()
    tr_hlist = list()
    ...
    with tf.Session() as session:
        ...
        for n_iter in range(iter_offset, args.max_iter):
            ...
            while nbatches_count > 0:
                ...
                hr_tlist, hr_tweight, tr_hlist, tr_hweight = data_generator_func(training_data_list[nbatches_count-1],model.tr_h, model.hr_t, model.n_entity, args.neg_weight)
